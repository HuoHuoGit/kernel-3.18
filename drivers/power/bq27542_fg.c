 ...
 * Datasheets:
 */
#define pr_fmt(fmt)	"bq27542- %s: " fmt, __func__
...
#define	FG_FLAGS_OT				BIT(13)
#define	FG_FLAGS_UT				BIT(12)
#define	FG_FLAGS_FC				BIT(9)
#define	FG_FLAGS_CHG				BIT(3)
...
static int fg_get_seal_state(struct bq_fg_chip *bq)
{
	...
	if (!(status & 0x4000) && !(status & 0x2000))
...
static int fg_read_df_version(struct bq_fg_chip* bq, u8 *ver)
{
	int ret;
	u16 dm_code = 0;
...
static int fg_df_pre_access(struct bq_fg_chip *bq)
{
	...
	if (ret)
		return ret;
	bq->seal_state_saved = bq->seal_state;
...
EXPORT_SYMBOL_GPL(fg_df_pre_access);

static int fg_df_post_access(struct bq_fg_chip *bq)
{
	int ret = 0;

	if (bq->seal_state_saved == SEAL_STATE_SEALED)
		ret = fg_seal(bq);

	return 0;
}
EXPORT_SYMBOL_GPL(fg_df_post_access);
...
static int fg_check_update_necessary(struct bq_fg_chip *bq)
{
	...
	if (!ret && df_ver < bqfs_image[bq->batt_id].version)
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = fg_df_read_block(bq, 82, 0, rd_buf);	//Qmax, offset 0
	if (ret) {
		fg_df_post_access(bq);
		mutex_unlock(&bq->update_lock);
		return 0;
	...
	ret = fg_df_read_block(bq, 88, 0, rd_buf);	//Ra Table
	if (ret) {
		fg_df_post_access(bq);
		mutex_unlock(&bq->update_lock);
		return idx;
...
static irqreturn_t fg_irq_thread(int irq, void *dev_id)
{
	...
	pr_err("RSOC:%d, Volt:%d, Current:%d, Temperature:%d, connected_rid = %d\n",
		bq->batt_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp, bq->connected_rid);
